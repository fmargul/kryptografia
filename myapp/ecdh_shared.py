...
def validate_ecdh_shared(p, a, b, X, Y, A):
    if p == None or a == None or b == None or X == None or Y == None or A == None:
    ...
    if Y != None and X != None:
        if ((Y*Y) % p != (pow(X, 3, p) + a*X + b) % p):
            return False, f"Punkt ({X}, {Y}) nie należy do krzywej."
    if not (1 < A < p):
       return False, "Liczba A musi być większa od 1 i mniejsza od p!"

    return True, [p, a, b, X, Y, A]

def calculate_ecdh_shared(p, a, b, X, Y, A):
    G = X, Y
